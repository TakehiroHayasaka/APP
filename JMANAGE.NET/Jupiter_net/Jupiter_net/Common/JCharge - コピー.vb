...
Module JCharge
    ...
    Function funチャージコードの有無新規工事(ccode As String, ByRef kogo As String, ByRef kozo As String) As Short
        '---------------------------------------------------------------------------------------------------
        '機能
        '   工号の下にある工事別ﾌﾟﾛﾌｧｲﾙからﾁｬｰｼﾞｺｰﾄﾞを読み込み、
        '   ﾁｬｰｼﾞｺｰﾄﾞ登録ﾌｧｲﾙに登録されているか判定する。
        ...
        If ccode = "NO-CHARGE" Then
            '工事別ﾌﾟﾛﾌｧｲﾙに登録ﾌｧｲﾙﾚｺｰﾄﾞ位置を書き込む。
            ret = WritePrivateProfileString("チャージコード", "登録ファイルレコード位置", CStr(funチャージコードの有無新規工事), ProFile)
            Exit Function
        End If
