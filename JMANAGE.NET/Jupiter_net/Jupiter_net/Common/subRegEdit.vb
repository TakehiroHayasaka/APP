...
#End If
	...
	Sub setDwgprefixForIntelliCAD(ByRef KojiPath As String)
        Dim regEditProfiles As String
		'    Dim status63,status64 As Long
		Dim status As Integer
		
		'    setKeyStringValue(HKEY_CURRENT_USER, "Software\Cosmo\IntelliCAD 6.4 Professional\Profiles\Default\Config", "Drawing", KojiPath)
		regEditProfiles = FunGetRegEditProfiles()
		status = setKeyStringValue(HKEY_CURRENT_USER, regEditProfiles & "Default\Config", "Drawing", KojiPath)
		
		If status <> 0 Then
			MsgBox("setDwgprefixForIntelliCAD Error!" & regEditProfiles)
		End If
