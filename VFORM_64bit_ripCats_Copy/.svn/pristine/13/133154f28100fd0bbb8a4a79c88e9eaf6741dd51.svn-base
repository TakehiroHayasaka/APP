Public Class BAmain
    ...
    Public Sub BA_Hontai()
        ...
        For i = 1 To 10
            ...
            '並列化動作確認用
            'Dim sw As New System.IO.StreamWriter("C:\01_VFORM_Projects\Calc_dEmat_para\dEmat" + CStr(N) + ".txt", False, System.Text.Encoding.GetEncoding("shift_jis"))
            'Dim cnt As Integer
            'For cnt = 0 To objBAmain.dEmat.Length - 1
            'sw.WriteLine(objBAmain.dEmat(cnt))
            'Next
            'sw.Close()
            ...
            'Dim sw1 As New System.IO.StreamWriter("C:\01_VFORM_Projects\BaDataCopy_Badata\obj" + CStr(N) + ".txt", False, System.Text.Encoding.GetEncoding("shift_jis"))
            'objBAmain.WriteObj(sw1)
            'sw1.Close()
            'Dim sw2 As New System.IO.StreamWriter("C:\01_VFORM_Projects\BaDataCopy_BaKoushin\obj" + CStr(N) + ".txt", False, System.Text.Encoding.GetEncoding("shift_jis"))
            'objBAKoushin.WriteObj(sw2)
            'sw2.Close()
    ...
    Public Sub SetInitData(ByVal CamPar As Object, ByVal INum As Integer, ByVal PNum As Integer, ByVal dblF0 As Double, ByVal flgSyori As Integer, ByVal iCameraNum As Integer)
        CamParam = CamPar
        ImageNum = INum
        PointNum = PNum
        CameraNum = iCameraNum
        F0 = dblF0
        '  CamParamToCamMat()
        If flgSyori = 1 Then
            CamGaibuHensu = 6
