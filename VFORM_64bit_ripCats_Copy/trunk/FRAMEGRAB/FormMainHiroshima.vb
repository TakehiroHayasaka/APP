...
Public Class FrmMainHiroshima
    ...
    Public Declare Ansi Function WritePrivateProfileInt Lib "kernel32.dll" Alias "WritePrivateProfileIntA" (ByVal lpApplicationName As String, ByVal lpKeyName As String, ByVal nDefault As Integer, ByVal lpFileName As String) As Integer
    ...
    Private Sub Button5_Click(sender As Object, e As EventArgs) Handles Button5.Click
        ...
        'ClearOfSetRecalcVal()
        Dim objFrmReg As New FrmReg_hiroshima
...
#If blnShinMeiwa = True Then
        ...
        Me.Text = "車両寸法測定システム " & "Build" & My.Settings.BuildNo
        'Add End
...
#Const Hiroshima = 3
#If Hiroshima = 1 Then          '20170713 SUURI TEST 用
...
#ElseIf Hiroshima = 2 Then
            ...
            Op.SetPart(AxHWindowXCtrl11.HalconWindow.HalconID, CInt(intH / objLiveParam.ImageHeight) * (objLiveParam.ImageHeight - 1), CInt(intW / objLiveParam.ImageWidth) * (objLiveParam.ImageWidth - 1), intH, intW)
            Op.SetPart(AxHWindowXCtrl12.HalconWindow.HalconID, CInt(intH / objLiveParam.ImageHeight) * (objLiveParam.ImageHeight - 1), CInt(intW / objLiveParam.ImageWidth) * (objLiveParam.ImageWidth - 1), intH, intW)
            Op.SetPart(AxHWindowXCtrl7.HalconWindow.HalconID, CInt(intH / objLiveParam.ImageHeight) * (objLiveParam.ImageHeight - 1), CInt(intW / objLiveParam.ImageWidth) * (objLiveParam.ImageWidth - 1), intH, intW)
            Op.SetPart(AxHWindowXCtrl8.HalconWindow.HalconID, CInt(intH / objLiveParam.ImageHeight) * (objLiveParam.ImageHeight - 1), CInt(intW / objLiveParam.ImageWidth) * (objLiveParam.ImageWidth - 1), intH, intW)

#ElseIf Hiroshima = 3 Then
...
#ElseIf Hiroshima = 4 Then
            Op.SetPart(AxHWindowXCtrl7.HalconWindow.HalconID, CInt(intH / objLiveParam.ImageHeight), CInt(intW / objLiveParam.ImageWidth), 0, 0)
            Op.SetPart(AxHWindowXCtrl8.HalconWindow.HalconID, 0, 0, CInt(intH / objLiveParam.ImageHeight), CInt(intW / objLiveParam.ImageWidth))
            Op.SetPart(AxHWindowXCtrl9.HalconWindow.HalconID, 0, 0, CInt(intH / objLiveParam.ImageHeight), CInt(intW / objLiveParam.ImageWidth))
            Op.SetPart(AxHWindowXCtrl10.HalconWindow.HalconID, 0, 0, CInt(intH / objLiveParam.ImageHeight), CInt(intW / objLiveParam.ImageWidth))
            Op.SetPart(AxHWindowXCtrl11.HalconWindow.HalconID, 0, 0, CInt(intH / objLiveParam.ImageHeight), CInt(intW / objLiveParam.ImageWidth))
...
#If blnDotNet Then
            Try
                ...
                Op.InfoFramfegrabber(cam, par, hv_ParameterInfo, hv_ParameterInfoValues)
            Catch ex As Exception
                '    MsgBox(ex.Message)
            End Try
...
#End If
        ...
        '  txtLastBasePointGetTime.Text = My.Settings.LastGetBasePointDate
        ...
        txtLastBasePointGetTime.Text = lstCamPose(0).CUpdateDate
    ...
    Private Sub ShootImageGetTarget(ByVal objCam As CameraParam)
        ...
        Dim TGParam As New TargetDetectParam
        TGParam.RoadFourTenTargetTxt()
        ...
        With objShootParam
            ...
            For i = .exposure_min To .exposure_max Step .exposure_kankaku
                ...
                Op.DispObj(ho_Image, hvWinHand)
#Const New4TGDetect = 1
#If New4TGDetect = 1 Then
                tmptarget.DetectTargetsOther2(indexResultImage, ho_Image, hv_ALLCTID, -1, 0, TGParam)
#Else
                tmptarget.DetectTargetsOther(indexResultImage, ho_Image, hv_ALLCTID, -1, 0)
...
#End If
                ...
                For j = .targetthreshold_min To .targetthreshold_max Step .targetthreshold_kankaku
                    tmptarget = New TargetDetect
#If New4TGDetect = 1 Then
                    tmptarget.DetectTargetsOther2(indexResultImage, ho_Image, hv_ALLCTID, j, 0, TGParam)
#Else
                    tmptarget.DetectTargetsOther(indexResultImage, ho_Image, hv_ALLCTID, j, 0)
#End If
...
#End If
    ...
    Public Sub ReadCamParamList()
        ...
        strSqlText = "SELECT CamParamID,CamParamFile,SerialNo FROM CamParamList WHERE FlgType = 3 ORDER BY CamParamID"
...
#End If
    ...
    Private Sub btnSatsueiSokuMode3_Click(sender As Object, e As EventArgs) Handles btnSatsueiSoku.Click

        'Dim SatsueiMode As Integer
        SatsueiMode = 3
        WritePrivateProfileString("Kaiseki", "CalcPoseByBasePoint", "0", My.Settings.YCMFolder & "\vform.ini")
        SatsueiSoku(SatsueiMode)
        BtnOffsetReCalc_Click(Nothing, Nothing)

    End Sub

    Private Sub btnSatueiSokuMode2_Click(sender As System.Object, e As System.EventArgs) Handles btnSatueiSokuMode2.Click

        'Dim SatsueiMode As Integer
        SatsueiMode = 2
        WritePrivateProfileString("Kaiseki", "CalcPoseByBasePoint", "1", My.Settings.YCMFolder & "\vform.ini")
        SatsueiSoku(SatsueiMode)
    ...
    Private Sub SatsueiSoku(ByVal CalcGaibuParam As Integer)
        ' Timer1.Stop()
        '連続して押した場合の対処
...
#If DebugTest = True Then
            ...
            FolderBrowserDialog1.SelectedPath = My.Settings.LastDebugKoujiFolder '"C:\01_VFORM_Projects\SUEOKI_DEBUG"
            ...
            Me.Text = Me.Text + " DebugDate " + strNewKoujiFolder
            My.Settings.LastDebugKoujiFolder = strNewKoujiFolder
        ...
        If flgKaisekiKahi = True Then
            ...
            If Int(CalcGaibuParam) = 3 Then
                ...
                If ConnectDbFBM(strNewKoujiFolder & "\") = True Then
                    'ReadCameraPose()
                    Dim SavedDate As String = Now.ToString("yyyy/MM/dd HH:mm:ss")
                    SaveCameraPose(SavedDate)
                    MonSavePose(strNewKoujiFolder & "\MonKaisekiMae.txt")
                End If
            End If
            'SUSANO ADD END

            KaisekiAndView(CalcGaibuParam)

            If CalcGaibuParam = 2 Then
                If CInt(txtBaseNinsiki.Text) = 100 Then
                    'My.Settings.LastGetBasePointDate = Now.ToString
                    Dim SavedDate As String = Now.ToString("yyyy/MM/dd HH:mm:ss")
                    'txtLastBasePointGetTime.Text = My.Settings.LastGetBasePointDate
                    GrpNGBasePointGetTime.Visible = False
                ...
                Else
                    txtNGBasePointGetTime.Text = Now.ToString
                    GrpNGBasePointGetTime.Visible = True
                    txtNGBasePointGetTime.ForeColor = Color.Red
...
#End If
        ...
        '
        Dim Result3Dpoint As New frmMeasureResult
        Result3Dpoint.LoadInfo(strNewKoujiFolder)
        Dim strHeadertxt1 = "測点名,内部番号,認識,X,Y,Z,ΔX,ΔY,ΔZ"
        Dim strHeadertxt2 = "測点名,内部番号,認識,X,Y,Z"

        Result3Dpoint.SaveCSV_fromDataGridView_InFname(strNewKoujiFolder & "\kijunCSV.csv", Result3Dpoint.DataGridView1, strHeadertxt1)
        Result3Dpoint.SaveCSV_fromDataGridView_InFname(strNewKoujiFolder & "\sokutenCSV.csv", Result3Dpoint.DataGridView2, strHeadertxt2)
    End Sub

    '////////////////////////////////

    Public Sub KaisekiAndView(ByVal CalcGaibuParam As Integer)
        ...
        'Shell("YCM.exe " & strNewKoujiFolder & " 1", AppWinStyle.Hide, True)

        'Shellから変更
        Dim p As New System.Diagnostics.Process()
        p.StartInfo.FileName = "YCM.exe"
        p.StartInfo.WorkingDirectory = My.Settings.YCMFolder
        p.StartInfo.Arguments = strNewKoujiFolder & " 1"
        p.Start()
        p.WaitForExit()
        ...
        objMeasureInfo.SetCalcGaibuParam(CalcGaibuParam) 'Add Kiryu 撮影モードの設定
    ...
    Public Sub putDataTpDataGrids(ByVal lstSokuteiData As List(Of SunpoSetTable))
        ...
        For Each Sokutei As SunpoSetTable In lstSokuteiData
            ...
            Else
                ...
                objVal2(1) = Math.Round(CDbl(Sokutei.KiteiVal), 1)
                objVal2(2) = Math.Round(CDbl(Sokutei.SunpoVal) - CDbl(Sokutei.KiteiVal), 1) 'SUURI 計測値から規定値を引く
...
#End If
        ...
        frmObjMeasureResult.SetCalcGaibuParam(SatsueiMode)
...
#End If
    ...
    Private Sub AxHWindowXCtrl1_ClickEvent(sender As Object, e As EventArgs) Handles AxHWindowXCtrl1.ClickEvent
        PrevImageCid = 7
        labPrevCamID.Text = PrevImageCid.ToString
    ...
    Private Sub AxHWindowXCtrl3_ClickEvent(sender As Object, e As EventArgs)
        PrevImageCid = 9
        labPrevCamID.Text = PrevImageCid.ToString
    ...
    Private Sub AxHWindowXCtrl4_ClickEvent(sender As Object, e As EventArgs)
        PrevImageCid = 10
        labPrevCamID.Text = PrevImageCid.ToString
    ...
    Private Sub AxHWindowXCtrl4_Enter(sender As Object, e As EventArgs)

    End Sub

    Private Sub AxHWindowXCtrl5_ClickEvent(sender As Object, e As EventArgs)
        PrevImageCid = 11
        labPrevCamID.Text = PrevImageCid.ToString
    ...
    Private Sub AxHWindowXCtrl5_Enter(sender As Object, e As EventArgs)

    End Sub

    Private Sub AxHWindowXCtrl6_ClickEvent(sender As Object, e As EventArgs)
        PrevImageCid = 12
        labPrevCamID.Text = PrevImageCid.ToString
    ...
    Private Sub AxHWindowXCtrl6_Enter(sender As Object, e As EventArgs)
    ...
    '(20170728 kiryu Add) -ハルコンウィンドウのアスペクト比を維持してリサイズ-
    Private Sub KeepAspectReSize()
        ...
        Try
            ...
            'SplitContainer2.Height = (SplitContainer2.SplitterDistance + 15) * 2

            'SplitContainer2.SplitterDistance = (Bairitu * AxHWindowXCtrl1.Size.Width)


            SplitContainer2.Height = (Bairitu * AxHWindowXCtrl1.Size.Width + 15) * 2
            SplitContainer2.SplitterDistance = SplitContainer2.Height / 2

            GrpLiveCamera.Height = 500 'SplitContainer2.Height

            SC_Main.SplitterDistance = 500 'SplitContainer2.Height + 30

            Dim asp As Double = AxHWindowXCtrl1.Size.Height / AxHWindowXCtrl1.Size.Width
            Dim asp2 As Double = AxHWindowXCtrl9.Size.Height / AxHWindowXCtrl9.Size.Width

            'SplitContainer5.SplitterDistance = (Bairitu * AxHWinPrev.Size.Width)
            'SplitContainer5.Width = AxHWinPrev.Height * (4 / 3)
            'GrpKaisekiImg.Height = GrpLiveCamera.Height
            SplitContainer3.Height = GrpLiveCamera.Height - 3
            SplitContainer5.SplitterDistance = AxHWinPrev.Width * Bairitu
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
        ...
        Dim asp3 As Double = AxHWinPrev.Size.Height / AxHWinPrev.Size.Width
    ...
    Private Sub btnSettei_Click(sender As System.Object, e As System.EventArgs) Handles btnSettei.Click
        'Timer1.Stop()
        ' frmBasePointInfo.ShowDialog()
        frmBasePointInfo.SetTargetWindowClass(Me)
        frmBasePointInfo.ShowDialog()
        ConnectDbFBM(My.Application.Info.DirectoryPath & "\")
        GetALLShootParam()
        cbShootParam.Items.Clear()
...
#End Region
    ...
    Private Sub MirrorImage(ByVal objCamParam As CameraParam, ByRef objDispImage As HalconDotNet.HImage)
        If objCamParam.Cid >= 1 And objCamParam.Cid <= 4 Then
    ...
    Private Sub BtnOffsetReCalc_Click(sender As System.Object, e As System.EventArgs) Handles BtnOffsetReCalc.Click

        Dim OnDgOffsetR As Double
        Dim OnDgOffsetL As Double

        Dim OnDgSideEnterR As Double
        Dim OnDgSideEnterL As Double

        Try

            OnDgOffsetR = CDbl(dgMeasureResult.Item(1, 4).Value)
            OnDgOffsetL = CDbl(dgMeasureResult.Item(1, 5).Value)

            OnDgSideEnterR = CDbl(dgMeasureResult.Item(1, 10).Value)
            OnDgSideEnterL = CDbl(dgMeasureResult.Item(1, 11).Value)

        Catch ex As Exception
            '初回の計測前に押されたとき、DGが無ければ終了
            Exit Sub
        End Try


        CheckBlankText(txtNbkFrontToNbkSntrR.Text)
        CheckBlankText(txtNbkFrontToNbkSntrL.Text)
        CheckBlankText(txtTireToWheelR.Text)
        CheckBlankText(txtTireToWheelL.Text)


        Dim TxtFormat As String = "#0"
        Try

            txtOffsetValR.Text = (OnDgOffsetR - CDbl(txtNbkFrontToNbkSntrR.Text)).ToString(TxtFormat)
            txtOffsetValL.Text = (OnDgOffsetL - CDbl(txtNbkFrontToNbkSntrL.Text)).ToString(TxtFormat)

            txtSideEnterR.Text = (OnDgSideEnterR - CDbl(txtTireToWheelR.Text)).ToString(TxtFormat)
            txtSideEnterL.Text = (OnDgSideEnterL - CDbl(txtTireToWheelL.Text)).ToString(TxtFormat)


        Catch ex As Exception
            MsgBox("数値以外が入力されました。数値を再入力してください。", MessageBoxIcon.Error)
            Exit Sub
        End Try
        ...
        Try
            ...
            'clsSunpoSet.m_dbClass.Connect("C:\01_VFORM_Projects\SUEOKI_DEBUG\2017082０_西日強\Data20170820_151510" & "\計測データ.mdb")
            clsSunpoSet.m_dbClass.Connect(strNewKoujiFolder & "\計測データ.mdb")


            Dim strSql As String
            If CDbl(txtOffsetValR.Text) > CDbl(txtOffsetValL.Text) Then
                strSql = "UPDATE SunpoSet Set ExSunpoVal = " & txtOffsetValR.Text & " WHERE SunpoID = 12"
                clsSunpoSet.m_dbClass.ExecuteSQL(strSql)
                strSql = "UPDATE SunpoSet Set ExSunpoVal = " & txtOffsetValL.Text & " WHERE SunpoID = 13"
                clsSunpoSet.m_dbClass.ExecuteSQL(strSql)
            Else
                strSql = "UPDATE SunpoSet Set ExSunpoVal = " & txtOffsetValL.Text & " WHERE SunpoID = 12"
                clsSunpoSet.m_dbClass.ExecuteSQL(strSql)
                strSql = "UPDATE SunpoSet Set ExSunpoVal = " & txtOffsetValR.Text & " WHERE SunpoID = 13"
                clsSunpoSet.m_dbClass.ExecuteSQL(strSql)
            End If

            strSql = "UPDATE SunpoSet Set ExSunpoVal = " & txtSideEnterR.Text & " WHERE SunpoID = 18"
            clsSunpoSet.m_dbClass.ExecuteSQL(strSql)
            strSql = "UPDATE SunpoSet Set ExSunpoVal = " & txtSideEnterL.Text & " WHERE SunpoID = 19"
            clsSunpoSet.m_dbClass.ExecuteSQL(strSql)
            ...
            strSql = "UPDATE KihonInfo Set ItemValue = " & txtNbkFrontToNbkSntrR.Text & " WHERE ID = 178"
    ...
    Private Sub CheckBlankText(ByRef str As String)
        If str = "" Then
            str = "0"
        End If
    End Sub

    Private Sub ClearOffSetRecalcVal()

        txtNbkFrontToNbkSntrR.Text = "0"
        txtNbkFrontToNbkSntrL.Text = "0"
        txtTireToWheelR.Text = "0"
        txtTireToWheelL.Text = "0"

        txtOffsetValR.Text = "0"
        txtOffsetValL.Text = "0"

        txtSideEnterR.Text = "0"
        txtSideEnterL.Text = "0"

    End Sub


    Private Sub txtNbkFrontToNbkSntrR_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtNbkFrontToNbkSntrR.TextChanged
        txtNbkFrontToNbkSntrL.Text = txtNbkFrontToNbkSntrR.Text
    End Sub

    Private Sub txtNbkFrontToNbkSntrR_Click(sender As System.Object, e As System.EventArgs) Handles txtNbkFrontToNbkSntrR.Click
        TxtBoxSinglClicAllSelect(txtNbkFrontToNbkSntrR)
    End Sub

    Private Sub txtTireToWheelR_Click(sender As System.Object, e As System.EventArgs) Handles txtTireToWheelR.Click
        TxtBoxSinglClicAllSelect(txtTireToWheelR)
    End Sub

    Private Sub txtTireToWheelL_Click(sender As System.Object, e As System.EventArgs) Handles txtTireToWheelL.Click
        TxtBoxSinglClicAllSelect(txtTireToWheelL)
    End Sub

    Private Sub TxtBoxSinglClicAllSelect(ByRef txtBox As TextBox)
        txtBox.SelectAll()
    End Sub
