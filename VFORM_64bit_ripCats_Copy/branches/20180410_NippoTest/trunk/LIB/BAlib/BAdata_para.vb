...
Public Class BAdata
    ...
    Public CNP As New CamNaibuParam
    ...
    Public Sub ResetEmat()
        ...
        GC.Collect()
        Shikisuu = PointHensu * PointNum + CamGaibuHensu * ImageNum + CamNaibuHensu
    ...
    Public Sub ResetdE_E_F_Gmat()
        ...
        For i As Integer = 0 To PointNum - 1
            Fmat(i) = New Double(PointHensu - 1, CamGaibuHensu * ImageNum + CamNaibuHensu - KoteiHensu - 1) {}
        ...
        ReDim Gmat(CamGaibuHensu * ImageNum + CamNaibuHensu - KoteiHensu - 1, CamGaibuHensu * ImageNum + CamNaibuHensu - KoteiHensu - 1)
    ...
    Public Sub Reconst3D()
        ...
        For i = 1 To PointNum
            ...
            For j = 1 To ImageNum
                ...
                End If
            Next
            Dim Kakko1ID As Object = Nothing
            Dim Kakko2ID As Object = Nothing
            Dim Kakko3ID As Object = Nothing

            Op.CreateMatrix(2 * nn, 3, kakko1, Kakko1ID)
            Op.CreateMatrix(2 * nn, 1, kakko3, Kakko3ID)

            Op.SolveMatrix(Kakko1ID, "general", 0.00000000000000022204, Kakko3ID, Kakko2ID)
            Dim XYZa As Object = Nothing
            Op.GetFullMatrix(Kakko2ID, XYZa)
            Points(i).X = Tuple.TupleSelect(XYZa, 0)
            Points(i).Y = Tuple.TupleSelect(XYZa, 1)
            Points(i).Z = Tuple.TupleSelect(XYZa, 2)
        Next
    End Sub
    ...
    Private Sub CalcPQRUVDHXHYak(ByVal i As Integer, ByVal j As Integer, ByVal tmpPoint As Point3D)
        ...
        Uak(i, j) = (ImagePoints(i, j).x - CNP.U0) * CNP.Sx
        Vak(i, j) = (ImagePoints(i, j).y - CNP.V0) * CNP.Sy
        Dak(i, j) = Uak(i, j) * Uak(i, j) + Vak(i, j) * Vak(i, j)
        HXak(i, j) = Uak(i, j) + _
                     Uak(i, j) * (CNP.K1 * Dak(i, j) + CNP.K2 * Dak(i, j) * Dak(i, j) + CNP.K3 * Dak(i, j) * Dak(i, j) * Dak(i, j)) + _
                    +2 * CNP.P2 * Uak(i, j) * Vak(i, j) + _
                    CNP.P1 * (Dak(i, j) + 2 * Uak(i, j) * Uak(i, j))
        HYak(i, j) = Vak(i, j) + _
                     Vak(i, j) * (CNP.K1 * Dak(i, j) + CNP.K2 * Dak(i, j) * Dak(i, j) + CNP.K3 * Dak(i, j) * Dak(i, j) * Dak(i, j)) + _
                    +CNP.P2 * (Dak(i, j) + 2 * Vak(i, j) * Vak(i, j)) + _
                    2 * CNP.P1 * Vak(i, j) * Uak(i, j)
        HXak(i, j) = HXak(i, j) / CNP.Sx + CNP.U0
        HYak(i, j) = HYak(i, j) / CNP.Sy + CNP.V0
    End Sub
    ...
    Public Sub Calc_dEmat()
        ...
        Select Case CamNaibuHensu   '画像の内部パラメータに対する一次微分要素
            Case 3
                t = t + 1
                dEmat(t) = Calc_dE_dEpsK_Focal_AllImage()
                t = t + 1
                dEmat(t) = Calc_dE_dEpsK_U0_AllImage()
                t = t + 1
                dEmat(t) = Calc_dE_dEpsK_V0_AllImage()
            Case 6
                t = t + 1
                dEmat(t) = Calc_dE_dEpsK_Focal_AllImage()
                t = t + 1
                ...
                dEmat(t) = Calc_dE_dEpsK_V0_AllImage()
                t = t + 1
                dEmat(t) = Calc_dE_dEpsK_K1_AllImage()
                t = t + 1
                dEmat(t) = Calc_dE_dEpsK_K2_AllImage()
                t = t + 1
                dEmat(t) = Calc_dE_dEpsK_K3_AllImage()
            Case 8
                t = t + 1
                dEmat(t) = Calc_dE_dEpsK_Focal_AllImage()
                t = t + 1
                ...
                dEmat(t) = Calc_dE_dEpsK_K3_AllImage()
                t = t + 1
                dEmat(t) = Calc_dE_dEpsK_P1_AllImage()
                t = t + 1
                dEmat(t) = Calc_dE_dEpsK_P2_AllImage()
        End Select
        ...
        For i = 1 To PointNum
            For i1 = 1 To PointHensu
                For n1 = 1 To CamNaibuHensu
                    ...
                    'ddEmat(Ej, Ei) = ddEmat(Ei, Ej)
                    '  Ei = (i - 1) * PointHensu + i1
                    Ej = ImageNum * CamGaibuHensu + n1 - KoteiHensu - 1
                    ' Fmat(Ei, Ej) = Calc_ddE_dEpsKL_PointsToImageNaibu(i, i1, n1)
                    'Fmat(Ei)(Ej) = Calc_ddE_dEpsKL_PointsToImageNaibu(i, i1, n1)
                    'lstFmat(i - 1)(i1 - 1, Ej) = Calc_ddE_dEpsKL_PointsToImageNaibu(i, i1, n1)
                    Fmat(i - 1)(i1 - 1, Ej) = Calc_ddE_dEpsKL_PointsToImageNaibu(i, i1, n1)
                Next
            Next
        Next
    End Sub
...
#End If
    ...
    Public Sub Calc_DeltaMat_yti(ByVal c As Double, ByRef arrDeltaMat() As Double)
        ...
        'Dim FallMatID As Object = Nothing
        '  Dim EallMatID As Object = Nothing
        'Dim FaMatID As Object = Nothing
        'Dim EaInvMatID As Object = Nothing
        'Dim DeltaAEMatID As Object = Nothing
        'Dim arrFaMatID(PointNum) As Object
        Dim arrEaInvMatID(PointNum)(,) As Double
        Dim arrDeltaAEMatID(PointNum)(,) As Double

        '  Dim DeltaEpsPMatID As Object = Nothing
        Dim LeftMatID(,) As Double = {}
        Dim RightMatID(,) As Double = {}
        'Dim SumLeftMatID As Object = Nothing
        'Dim SumRightMatID As Object = Nothing
        'Dim MultMatId1 As Object = Nothing
        'Dim MultMatId2 As Object = Nothing
        'Dim AddMatID As Object = Nothing
        ...
        StopWatchResetStart()
        '  Op.SubMatrix(GMatID, SumLeftMatID, LeftMatID)
        MSub(GMatID, arrSumLeftMatID, LeftMatID)

        'Op.SubMatrix(SumRightMatID, DFMatID, RightMatID)
        ...
        ' Op.CreateMatrix(PointHensu, 1, -1.0, tmpMatID)
        ...
        For i = 1 To PointNum
            ...
            'Op.MultMatrix(FaMatID, DeltaEpsFMatID, "AB", MultMatId1)
            MMult(Fmat(i - 1), arrDeltaEpsFMatID, MultMat1)

            'Op.AddMatrix(MultMatId1, DeltaAEMatID, AddMatID)
            MAdd(MultMat1, DeltaAEMatID, AddMat)

            ' Op.MultMatrix(EaInvMatID, AddMatID, "AB", MultMatId2)
            MMult(EaInvMatID, AddMat, MultMat2)

            'Op.MultElementMatrixMod(MultMatId2, tmpMatID)
            MScaleMod(MultMat2, -1.0)

            'Op.SetSubMatrix(DeltaMatID, MultMatId2, (i - 1) * PointHensu, 0)
            For j = 0 To PointHensu - 1
                arrDeltaMat((i - 1) * PointHensu + j) = MultMat2(j, 0)
            Next

            'Op.ClearMatrix(MultMatId2)
            'Op.ClearMatrix(MultMatId1)
            'Op.ClearMatrix(DeltaAEMatID)
            'Op.ClearMatrix(EaInvMatID)
            ''Op.ClearMatrix(FaMatID)
            'Op.ClearMatrix(AddMatID)
        Next
        For j = PointHensu * PointNum To Shikisuu - KoteiHensu - 1
            arrDeltaMat(j) = arrDeltaEpsFMatID(j - PointHensu * PointNum, 0)
        Next

        ' Op.SetSubMatrix(DeltaMatID, DeltaEpsFMatID, PointNum * PointHensu, 0)

        'Op.CreateMatrix(Shikisuu - KoteiHensu, 1, arrDeltaMatID, DeltaMatID)

        JikanMonitor("バンドル更新値計算4:")

        'Op.ClearMatrix(GMatID)
        'Op.ClearMatrix(DFMatID)
        '  Op.ClearMatrix(FallMatID)
        '  Op.ClearMatrix(EallMatID)

        'Op.ClearMatrix(DeltaEpsPMatID)
        'Op.ClearMatrix(LeftMatID)
        'Op.ClearMatrix(RightMatID)
        'Op.ClearMatrix(SumLeftMatID)
        'Op.ClearMatrix(SumRightMatID)
        'Op.ClearMatrix(MultMatId1)
        'Op.ClearMatrix(MultMatId2)
    ...
    Private Sub Halcon_Solve(ByRef LeftMatId(,) As Double, ByRef RightMatID(,) As Double, ByRef arrDeltaEpsFMatID(,) As Double)
        ...
        Dim LeftMatID_hal As Object = Nothing
        Dim RigthMatID_hal As Object = Nothing
        Dim DeltaEpsFMatID As Object = Nothing
        Dim t As Integer = 0
        ...
        For i = 0 To nG - 1
            ...
            Next
        Next
        Op.CreateMatrix(nG, nG, tmpObj, LeftMatID_hal)

        ReDim tmpObj(nG - 1)
        For i = 0 To nG - 1
            tmpObj(i) = RightMatID(i, 0)
        Next
        Op.CreateMatrix(nG, 1, tmpObj, RigthMatID_hal)
        Try
            Op.SolveMatrix(LeftMatID_hal, "general", 0, RigthMatID_hal, DeltaEpsFMatID)
        Catch ex As Exception
            Op.SolveMatrix(LeftMatID_hal, "general", 0.00000000000000022204, RigthMatID_hal, DeltaEpsFMatID)
        End Try

        Op.GetFullMatrix(DeltaEpsFMatID, tmpObj)

        For i = 0 To nG - 1
            arrDeltaEpsFMatID(i, 0) = tmpObj(i)
        Next
        Op.ClearMatrix(DeltaEpsFMatID)
        Op.ClearMatrix(LeftMatID_hal)
        Op.ClearMatrix(RigthMatID_hal)
    End Sub
    Private Sub Calc_DF_Mat_yti(ByRef DF_MatID(,) As Double)
        ...
        ''GC.Collect()
        ''GC.WaitForPendingFinalizers()
        'Op.CreateMatrix(Shikisuu - KoteiHensu - PointNum * PointHensu, 1, tmpobj, DF_MatID)
    End Sub
    ...
    Public Sub Calc_DeltaMat(ByVal c As Double, ByRef DeltaMatID As Object)
        Dim GMatID As Object = Nothing
        Dim DFMatID As Object = Nothing
        'Dim FallMatID As Object = Nothing
        '  Dim EallMatID As Object = Nothing
        'Dim FaMatID As Object = Nothing
        'Dim EaInvMatID As Object = Nothing
        'Dim DeltaAEMatID As Object = Nothing
        Dim arrFaMatID(PointNum) As Object
        Dim arrEaInvMatID(PointNum) As Object
        Dim arrDeltaAEMatID(PointNum) As Object
        Dim DeltaEpsFMatID As Object = Nothing
        ' Dim DeltaEpsPMatID As Object = Nothing
        Dim LeftMatID As Object = Nothing
        Dim RightMatID As Object = Nothing
        Dim SumLeftMatID As Object = Nothing
        Dim SumRightMatID As Object = Nothing
        Dim MultMatId1 As Object = Nothing
        Dim MultMatId2 As Object = Nothing
        ...
        Op.CreateMatrix(Shikisuu - KoteiHensu, 1, 0.0, DeltaMatID)
        ...
        GC.Collect()
        GC.WaitForPendingFinalizers()
        Op.CreateMatrix(nG, nG, 0.0, SumLeftMatID)
        Op.CreateMatrix(nG, 1, 0.0, SumRightMatID)
        For i = 1 To PointNum
            Dim FaMatID As Object = Nothing
            Dim EaInvMatID As Object = Nothing
            Dim DeltaAEMatID As Object = Nothing
            ...
            Op.MultMatrix(FaMatID, EaInvMatID, "ATB", MultMatId1)
            Op.MultMatrix(MultMatId1, FaMatID, "AB", MultMatId2)
            Op.AddMatrixMod(SumLeftMatID, MultMatId2)
            Op.ClearMatrix(MultMatId2)
            Op.MultMatrix(MultMatId1, DeltaAEMatID, "AB", MultMatId2)
            Op.AddMatrixMod(SumRightMatID, MultMatId2)
            Op.ClearMatrix(MultMatId2)
        ...
        Op.SolveMatrix(LeftMatID, "general", 0.00000000000000022204, RightMatID, DeltaEpsFMatID)
    ...
    Public Sub Calc_DeltaMat_para(ByVal c As Double, ByRef DeltaMatID As Object)
        Dim GMatID As Object = Nothing
        Dim DFMatID As Object = Nothing
        Dim FallMatID As Object = Nothing
        Dim EallMatID As Object = Nothing
        ...
        Dim arrEaInvMatID(PointNum) As Object
        Dim arrDeltaAEMatID(PointNum) As Object
        Dim arrDeltaMatItemID(PointNum) As Object
        Dim DeltaEpsFMatID As Object = Nothing
        Dim DeltaEpsPMatID As Object = Nothing
        Dim LeftMatID As Object = Nothing
        Dim RightMatID As Object = Nothing
        Dim SumLeftMatID As Object = Nothing
        Dim SumRightMatID As Object = Nothing
        Dim arrSumLeftMatID(PointNum) As Object
        Dim arrSumRightMatID(PointNum) As Object
        ...
        Parallel.For(1, PointNum + 1, Sub(i)
                                          Dim MultMatId1 As Object = Nothing
                                          Dim MultMatId2 As Object = Nothing
                                          Dim MultMatId3 As Object = Nothing
                                          Dim FaMatID As Object = Nothing
                                          Dim EaInvMatID As Object = Nothing
                                          Dim DeltaAEMatID As Object = Nothing
                                          ...
                                          Op.MultMatrix(MultMatId1, DeltaAEMatID, "AB", MultMatId3)
                                          arrSumRightMatID(i) = MultMatId3
                                          Op.ClearMatrix(MultMatId1)
                                      End Sub)

        For i As Integer = 1 To PointNum
            Op.AddMatrixMod(SumLeftMatID, arrSumLeftMatID(i))
            Op.AddMatrixMod(SumRightMatID, arrSumRightMatID(i))
            Op.ClearMatrix(arrSumLeftMatID(i))
            Op.ClearMatrix(arrSumRightMatID(i))
        Next
        sw.Stop()
        Trace.WriteLine("バンドル更新値計算2：")
        Trace.WriteLine(sw.Elapsed.TotalSeconds & "秒")
        ...
        For i As Integer = 1 To PointNum
            Op.SetSubMatrix(DeltaMatID, arrDeltaMatItemID(i), (i - 1) * PointHensu, 0)
            Op.ClearMatrix(arrDeltaMatItemID(i))
        Next
    ...
    Public Sub Calc_DeltaMat2(ByVal c As Double, ByRef DeltaMatID As Object)
        Dim GMatID As Object = Nothing
        Dim DFMatID As Object = Nothing
        Dim arrFaMatID(PointNum) As Object
        Dim arrEaInvMatID(PointNum) As Object
...
#End If
        ...
        For i = 1 To PointNum
            Op.MultMatrix(arrFaMatID(i), arrEaInvMatID(i), "ATB", MultMatId1)
            Op.MultMatrix(MultMatId1, arrFaMatID(i), "AB", MultMatId2)
            Op.AddMatrixMod(SumLeftMatID, MultMatId2)
            Op.ClearMatrix(MultMatId2)
            Op.MultMatrix(MultMatId1, arrDeltaAEMatID(i), "AB", MultMatId2)
            Op.AddMatrixMod(SumRightMatID, MultMatId2)
            Op.ClearMatrix(MultMatId2)
            Op.ClearMatrix(MultMatId1)
        Next
...
#End If
        ...
        Op.SolveMatrix(LeftMatID, "general", 0, RightMatID, DeltaEpsFMatID)
...
#End If
        For i = 1 To PointNum
            Op.MultMatrix(arrFaMatID(i), DeltaEpsFMatID, "AB", MultMatId1)
            Op.AddMatrix(MultMatId1, arrDeltaAEMatID(i), AddMatID)
            Op.MultMatrix(arrEaInvMatID(i), AddMatID, "AB", MultMatId2)
            Op.MultElementMatrixMod(MultMatId2, tmpMatID)
            ...
            Op.ClearMatrix(MultMatId2)
            Op.ClearMatrix(MultMatId1)
            Op.ClearMatrix(arrDeltaAEMatID(i))
            Op.ClearMatrix(arrEaInvMatID(i))
            Op.ClearMatrix(arrFaMatID(i))
            Op.ClearMatrix(AddMatID)
        Next
...
#End If
    ...
    Private Sub Calc_G_mat(ByVal c As Double, ByRef GMatID As Object)
        Dim i As Integer
        Dim j As Integer
        ...
        'GC.Collect()
        'GC.WaitForPendingFinalizers()
        Op.CreateMatrix(Shikisuu - KoteiHensu - PointNum * PointHensu, Shikisuu - KoteiHensu - PointNum * PointHensu, tmpobj, GMatID)
    End Sub
    'Private Sub Calc_Fall_mat(ByRef arrFaMatID() As Object)
    '    Dim i As Integer
    '    Dim k As Integer
    '    Dim j As Integer
    '    Dim l As Integer = 1
    ...
    '        'GC.Collect()
    '        'GC.WaitForPendingFinalizers()
    '        Op.CreateMatrix(PointHensu, Shikisuu - KoteiHensu - PointNum * PointHensu, tmpobj, FaMatID)
    '        arrFaMatID(i) = FaMatID
    '    Next

    'End Sub

    'Private Sub Calc_Fall_mat(ByRef FallMatID As Object)
    '    Dim i As Integer
    '    Dim j As Integer
    '    Dim l As Integer = 1
    '    Dim t As Integer = 0
    ...
    '    Op.CreateMatrix(PointHensu * PointNum, Shikisuu - KoteiHensu - PointNum * PointHensu, tmpobj, FallMatID)
    'End Sub
    Private Sub Calc_Fa_mat(ByRef FallMatId As Object, ByVal a As Integer, ByRef FaMatID As Object)

        'GC.Collect()
        'GC.WaitForPendingFinalizers()
        Op.GetSubMatrix(FallMatId, (a - 1) * PointHensu, 0, _
                        PointHensu, Shikisuu - KoteiHensu - PointNum * PointHensu, FaMatID)

    End Sub
   

    Private Sub Calc_Fa_mat(ByVal a As Integer, ByRef FaMatID As Object)
        Dim i As Integer
        Dim j As Integer
        Dim t As Integer = 0
        'GC.Collect()
        ...
        Op.CreateMatrix(PointHensu, Shikisuu - KoteiHensu - PointNum * PointHensu, tmpobj, FaMatID)
    End Sub
    'Private Sub Calc_EaInvAll_mat(ByVal c As Double, ByRef arrEaInvMatID() As Object)
    '    Dim i As Integer
    '    Dim i1 As Integer
    '    Dim i2 As Integer
    '    Dim t As Integer = 0
    ...
    '            Next
    '        Next
    '        Dim EaMatID As Object = Nothing
    '        Op.CreateMatrix(PointHensu, PointHensu, tmpobj, EaMatID)
    '        Op.InvertMatrixMod(EaMatID, "general", 0)
    '        arrEaInvMatID(i) = EaMatID
    '    Next

    'End Sub
    'Private Sub Calc_EaInvAll_mat(ByVal c As Double, ByRef EallMatID As Object)
    ...
    '    Op.CreateMatrix(PointHensu * PointNum, PointHensu, tmpobj, EallMatID)

    'End Sub

    'Private Sub Calc_EaInv_mat(ByRef EallMatID As Object, ByVal a As Integer, ByRef EaMatID As Object)
    '    Op.GetSubMatrix(EallMatID, (a - 1) * PointHensu, 0, PointHensu, PointHensu, EaMatID)
    '    'Op.CreateMatrix(PointHensu, PointHensu, tmpobj, EaMatID)
    '    Op.InvertMatrixMod(EaMatID, "general", 0.00000000000000022204)

    'End Sub

    Private Sub Calc_EaInv_mat(ByVal c As Integer, ByVal a As Integer, ByRef EaMatID As Object)
        Dim i1 As Integer
        Dim i2 As Integer
        Dim t As Integer = 0
        'GC.Collect()
    ...
    'Private Sub Calc_EaInv_mat(ByVal c As Double, ByVal a As Integer, ByRef EaMatID As Object)
    '    Dim Ei As Integer
    '    Dim Ej As Integer
    '    Dim i1 As Integer
    '    Dim i2 As Integer
    ...
    Private Sub Calc_AllDelta_a_E_mat(ByRef arrDeltaAEMatID() As Object)
        Dim Ei As Integer
        Dim i As Integer
        Dim j As Integer
        Dim t As Integer = 0
        ...
        For i = 1 To PointNum
            ...
            For j = 1 To PointHensu
                ...
                t = t + 1
            Next
            Dim Delta_a_E_MatID As Object = Nothing
            Op.CreateMatrix(PointHensu, 1, tmpobj, Delta_a_E_MatID)
    ...
    Private Sub Calc_Delta_a_E_mat(ByVal a As Integer, ByRef Delta_a_E_MatID As Object)
        Dim Ei As Integer
        Dim i As Integer
        Dim t As Integer = 0
        Dim tmpobj(PointHensu - 1) As Double
    ...
    Private Sub Calc_DF_Mat(ByRef DF_MatID As Object)
        Dim Ei As Integer
        Dim i As Integer
        Dim l As Integer = 1
        Dim t As Integer = 0
    ...
    '    Public Sub Create_ddEmatID(ByVal c As Double, ByRef ddEMatID As Object)
    '        Dim i As Integer
    '        Dim j As Integer
    ...
    '                    tmpobj(t) = ddEmat(i, j)
    '                End If
    '                '   tmpobj = Tuple.TupleConcat(tmpobj, tmpData)
    '                t = t + 1
    'removeline1:
    '            Next
    'removeline:
    ...
    '        Next
    '        Op.CreateMatrix(Shikisuu - KoteiHensu, Shikisuu - KoteiHensu, tmpobj, ddEMatID)
    '    End Sub

    Public Sub Create_dEmatID(ByRef dEMatID As Object)
        Dim i As Integer
        Dim t As Integer
        Dim k As Integer = 0
        GC.Collect()
...
removeline:
        Next
        Op.CreateMatrix(Shikisuu - KoteiHensu, 1, tmpObj, dEMatID)
    ...
    Public Sub BaDataKoushin(ByRef BaMainData As BAdata, ByVal DeltaVal() As Double)
        ...
        Dim HomMat3dIdentity As Object = Nothing
        Op.HomMat3dIdentity(HomMat3dIdentity)

        ' Op.GetFullMatrix(DeltaMatID, DeltaVal)
        ...
        Select Case CamNaibuHensu
            Case 0
                Me.CNP.F = BaMainData.CNP.F
                Me.CNP.U0 = BaMainData.CNP.U0
                Me.CNP.V0 = BaMainData.CNP.V0
                Me.CNP.K1 = BaMainData.CNP.K1
                Me.CNP.K2 = BaMainData.CNP.K2
                Me.CNP.K3 = BaMainData.CNP.K3
                Me.CNP.P1 = BaMainData.CNP.P1
                Me.CNP.P2 = BaMainData.CNP.P2
            Case 3
                Me.CNP.F = BaMainData.CNP.F + DeltaVal(Shikisuu - 3 - KoteiHensu)
                Me.CNP.U0 = BaMainData.CNP.U0 + DeltaVal(Shikisuu - 2 - KoteiHensu)
                Me.CNP.V0 = BaMainData.CNP.V0 + DeltaVal(Shikisuu - 1 - KoteiHensu)
                Me.CNP.K1 = BaMainData.CNP.K1
                Me.CNP.K2 = BaMainData.CNP.K2
                Me.CNP.K3 = BaMainData.CNP.K3
                Me.CNP.P1 = BaMainData.CNP.P1
                Me.CNP.P2 = BaMainData.CNP.P2
            Case 6
                Me.CNP.F = BaMainData.CNP.F + DeltaVal(Shikisuu - 6 - KoteiHensu)
                Me.CNP.U0 = BaMainData.CNP.U0 + DeltaVal(Shikisuu - 5 - KoteiHensu)
                Me.CNP.V0 = BaMainData.CNP.V0 + DeltaVal(Shikisuu - 4 - KoteiHensu)
                Me.CNP.K1 = BaMainData.CNP.K1 + DeltaVal(Shikisuu - 3 - KoteiHensu)
                Me.CNP.K2 = BaMainData.CNP.K2 + DeltaVal(Shikisuu - 2 - KoteiHensu)
                Me.CNP.K3 = BaMainData.CNP.K3 + DeltaVal(Shikisuu - 1 - KoteiHensu)
                Me.CNP.P1 = BaMainData.CNP.P1
                Me.CNP.P2 = BaMainData.CNP.P2
            Case 8
                Me.CNP.F = BaMainData.CNP.F + DeltaVal(Shikisuu - 8 - KoteiHensu)
                Me.CNP.U0 = BaMainData.CNP.U0 + DeltaVal(Shikisuu - 7 - KoteiHensu)
                Me.CNP.V0 = BaMainData.CNP.V0 + DeltaVal(Shikisuu - 6 - KoteiHensu)
                Me.CNP.K1 = BaMainData.CNP.K1 + DeltaVal(Shikisuu - 5 - KoteiHensu)
                Me.CNP.K2 = BaMainData.CNP.K2 + DeltaVal(Shikisuu - 4 - KoteiHensu)
                Me.CNP.K3 = BaMainData.CNP.K3 + DeltaVal(Shikisuu - 3 - KoteiHensu)
                Me.CNP.P1 = BaMainData.CNP.P1 + DeltaVal(Shikisuu - 2 - KoteiHensu)
                Me.CNP.P2 = BaMainData.CNP.P2 + DeltaVal(Shikisuu - 1 - KoteiHensu)
        End Select

        Me.CNP.Sx = BaMainData.CNP.Sx
        Me.CNP.Sy = BaMainData.CNP.Sy
        Me.CNP.A = BaMainData.CNP.A
        ...
        For j = 1 To ImageNum
            ...
            If j = BaMainData.FirstImageIndex And KoteiHensu <> 0 Then
                ...
                '投影行列の更新
                Me.Images(j).P_wo_Koushin(Me.CNP)
                Continue For
            End If
            ...
            Dim Axis As Object = Me.Images(j).W.Axis
            Dim RWk As Object = Nothing
            Dim tmpR As New RMat
            Op.HomMat3dRotateLocal(HomMat3dIdentity, kakudo, Axis, RWk)
...
#Region "一次微分要素"
    ...
    Private Function Calc_dE_dEpsK_Focal_AllImage() As Double
        Dim i As Integer
        Dim j As Integer
        Dim dPak_dEpsk As Double
        Dim dQak_dEpsk As Double
    ...
    Private Function Calc_dE_dEpsK_U0_AllImage() As Double
        Dim i As Integer
        Dim j As Integer
        Dim dPak_dEpsk As Double
        Dim dQak_dEpsk As Double
    ...
    Private Function Calc_dE_dEpsK_V0_AllImage() As Double
        Dim i As Integer
        Dim j As Integer
        Dim dPak_dEpsk As Double
        Dim dQak_dEpsk As Double
    ...
    Private Function Calc_dE_dEpsK_K1_AllImage() As Double
        Dim i As Integer
        Dim j As Integer
        Dim dPak_dEpsk As Double
        Dim dQak_dEpsk As Double
    ...
    Private Function Calc_dE_dEpsK_K2_AllImage() As Double
        Dim i As Integer
        Dim j As Integer
        Dim dPak_dEpsk As Double
        Dim dQak_dEpsk As Double
    ...
    Private Function Calc_dE_dEpsK_K3_AllImage() As Double
        Dim i As Integer
        Dim j As Integer
        Dim dPak_dEpsk As Double
        Dim dQak_dEpsk As Double
    ...
    Private Function Calc_dE_dEpsK_P1_AllImage() As Double
        Dim i As Integer
        Dim j As Integer
        Dim dPak_dEpsk As Double
        Dim dQak_dEpsk As Double
    ...
    Private Function Calc_dE_dEpsK_P2_AllImage() As Double
        Dim i As Integer
        Dim j As Integer
        Dim dPak_dEpsk As Double
        Dim dQak_dEpsk As Double
...
#Region "二次微分要素"
    ...
    Private Function Calc_ddE_dEpsKL_GaibuToNaibu(ByVal ImageIndex As Integer, ByVal J1 As Integer, ByVal N2 As Integer) As Double
        Dim i As Integer
        Dim j As Integer
        Dim dPak_dEpsk As Double
        Dim dQak_dEpsk As Double
    ...
    Private Function Calc_ddE_dEpsKL_NaibuToNaibu(ByVal N1 As Integer, ByVal N2 As Integer) As Double
        Dim i As Integer
        Dim j As Integer
        Dim dPak_dEpsk As Double
        Dim dQak_dEpsk As Double
    ...
    Private Function Calc_ddE_dEpsKL_PointsToImageNaibu(ByVal PointIndex As Integer, ByVal I1 As Integer, ByVal N2 As Integer) As Double
        Dim i As Integer
        Dim j As Integer
        Dim dPak_dEpsk As Double
        Dim dQak_dEpsk As Double
...
#Region "Pak,Qak,Rakの焦点距離に対する一階微分"
    ...
    Private Function dPak_dF(ByVal a As Integer, ByVal k As Integer) As Double
        Return (Pak(a, k) - (CNP.U0 / F0) * Rak(a, k)) / CNP.F
    End Function

    Private Function dQak_dF(ByVal a As Integer, ByVal k As Integer) As Double
        Return (Qak(a, k) - (CNP.V0 / F0) * Rak(a, k)) / CNP.F  'SUURI 20111114 U0->V0に修正
    End Function
...
#Region "Pak,Qak,Rakの並進に対する一階微分"
    ...
    Private Function dPak_dT1(ByVal k As Integer) As Double
        ...
        Return (-1) * (CNP.A * CNP.F * Images(k).R.R11 + CNP.U0 * Images(k).R.R13) / F0

    End Function

    Private Function dQak_dT1(ByVal k As Integer) As Double

        Return (-1) * (CNP.F * Images(k).R.R12 + CNP.V0 * Images(k).R.R13) / F0
    ...
    Private Function dPak_dT2(ByVal k As Integer) As Double
        ...
        Return (-1) * (CNP.A * CNP.F * Images(k).R.R21 + CNP.U0 * Images(k).R.R23) / F0

    End Function

    Private Function dQak_dT2(ByVal k As Integer) As Double

        Return (-1) * (CNP.F * Images(k).R.R22 + CNP.V0 * Images(k).R.R23) / F0
    ...
    Private Function dPak_dT3(ByVal k As Integer) As Double
        ...
        Return (-1) * (CNP.A * CNP.F * Images(k).R.R31 + CNP.U0 * Images(k).R.R33) / F0

    End Function

    Private Function dQak_dT3(ByVal k As Integer) As Double

        Return (-1) * (CNP.F * Images(k).R.R32 + CNP.V0 * Images(k).R.R33) / F0
...
#Region "HXak,HYakをU0,V0,K1,K2,K3,P1,P2での一階微分"
    Private Function K123(ByVal a As Integer, ByVal k As Integer)
        Return CNP.K1 * Dak(a, k) + CNP.K2 * Dak(a, k) * Dak(a, k) + CNP.K3 * Dak(a, k) * Dak(a, k) * Dak(a, k)
    End Function
    Private Function dK123_dU0(ByVal a As Integer, ByVal k As Integer)
        Return CNP.K1 * dDak_dU0(a, k) + CNP.K2 * (2 * Dak(a, k) * dDak_dU0(a, k)) + CNP.K3 * (3 * Dak(a, k) * Dak(a, k) * dDak_dU0(a, k))
    End Function
    Private Function dK123_dV0(ByVal a As Integer, ByVal k As Integer)
        Return CNP.K1 * dDak_dV0(a, k) + CNP.K2 * (2 * Dak(a, k) * dDak_dV0(a, k)) + CNP.K3 * (3 * Dak(a, k) * Dak(a, k) * dDak_dV0(a, k))
    End Function
    Private Function dDak_dU0(ByVal a As Integer, ByVal k As Integer)
        Return 2 * Uak(a, k) * (-1) * CNP.Sx
    End Function
    Private Function dDak_dV0(ByVal a As Integer, ByVal k As Integer)
        Return 2 * Vak(a, k) * (-1) * CNP.Sy
    End Function
