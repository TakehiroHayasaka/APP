Public Class CodedTarget
    ...
    Public ReadOnly Property CenterPoint() As ImagePoints
        ...
        Get
            ...
            NewObj.Row = Tuple.TupleSelect(CT_Points.Row, 0)
            NewObj.Col = Tuple.TupleSelect(CT_Points.Col, 0)
            Return NewObj
        End Get
    ...
    Public Sub UpdateData()
        ...
        For i = 0 To 0
            CreateFieldText(i)
            If dbClass.DoUpdate(SingleTargetFields, "Targets", strFieldText, "ImageID = " & ImageID1 & " And P2ID = " & CT_ID & " And P3ID = 0 ") = -1 Then
                MsgBox("DB登録に失敗しました。", MsgBoxStyle.OkOnly, "エラー")
                Exit Sub
            End If
        Next
    End Sub
    Public Sub SaveDataAllTargets()
        Dim n As Integer = CTnoSTnum
        Dim i As Integer
        For i = 0 To n - 1
            CreateFieldText(i)
            If dbClass.DoInsert(SingleTargetFields, "AllTargets", strFieldText) < 0 Then
                MsgBox("DB登録に失敗しました。", MsgBoxStyle.OkOnly, "エラー")
                Exit Sub
            End If
        Next
    ...
    Public Sub DeleteDataS()
        If dbClass.DoDelete("Targets", "ImageID=" & ImageID1 & " AND P2ID=" & CT_ID) < 0 Then
            MsgBox("レコード削除に失敗しました。", MsgBoxStyle.OkOnly, "エラー")
            Exit Sub
        End If
    End Sub
    '20150130 ADD By Yamada sta----------------------'SUURI　質問　DELETEDATA関数なのに何で中にINSERTしているの？
    ''' <summary>
    ''' 
    ''' </summary>
    ...
    Public Sub ReadData(ByRef IDR As IDataReader)
        Dim n As Integer = CTnoSTnum
        Dim i As Integer
        ImageID1 = CInt(IDR.GetValue(0))
        CT_ID = CInt(IDR.GetValue(1))
        flgUsable = CBool(IDR.GetValue(2))
        CT_Points.Row = DBNull.Value
        CT_Points.Col = DBNull.Value
        CT_Points.Row = New Object()
        CT_Points.Col = New Object()
        ReDim CT_Points.Row(n - 1)
        ReDim CT_Points.Col(n - 1)
        For i = 0 To n - 1
            Dim R As Object
            Dim C As Object
            ...
            'CT_Points.Row = Tuple.TupleConcat(CT_Points.Row, R)
            'CT_Points.Col = Tuple.TupleConcat(CT_Points.Col, C)
            CT_Points.Row(i) = R
            CT_Points.Col(i) = C
