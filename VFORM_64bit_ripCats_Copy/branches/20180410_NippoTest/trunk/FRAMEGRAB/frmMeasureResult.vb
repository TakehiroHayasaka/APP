...
Public Class frmMeasureResult
    ...
    Private CalcGaibuParam As Integer = 2
...
#If LeadMDBOnly = 1 Then
        ...
        My.Settings.LastOpenKesokuMDB = strNewKoujiFolder
...
#End If
        ...
        If ConnectDbSystemSetting(sysPath) Then
            ...
            Dim filename As String = My.Application.Info.DirectoryPath & "\Setting\ShowBasePoint.csv"
            Dim fields As String()
            Dim delimiter As String = ","
            Dim ListShowBasePoint As New List(Of String())
#Const FreeOmomi = 1
#If FreeOmomi Then
            ...
            Dim MotoBasePointX(lstBasePoint3d.Count - 1) As Double
            Dim MotoBasePointY(lstBasePoint3d.Count - 1) As Double
            Dim MotoBasePointZ(lstBasePoint3d.Count - 1) As Double

            Dim HikakuBasePointX(lstBasePoint3d.Count - 1) As Double
            Dim HikakuBasePointY(lstBasePoint3d.Count - 1) As Double
            Dim HikakuBasePointZ(lstBasePoint3d.Count - 1) As Double

            Dim Xweight(lstBasePoint3d.Count - 1) As Double
            Dim Yweight(lstBasePoint3d.Count - 1) As Double
            Dim Zweight(lstBasePoint3d.Count - 1) As Double

            Dim RetBasePointX(lstBasePoint3d.Count - 1) As Double
            Dim RetBasePointY(lstBasePoint3d.Count - 1) As Double
            Dim RetBasePointZ(lstBasePoint3d.Count - 1) As Double
            ...
            For Each objBasePoint As measurepoint3dTable In lstBasePoint3d
                For Each Sokutei As measurepoint3dTable In lstmeasurepoint3d
                    If objBasePoint.systemlabel = Sokutei.systemlabel Then
                        MotoBasePointX(i) = CDbl(objBasePoint.X)
                        MotoBasePointY(i) = CDbl(objBasePoint.Y)
                        MotoBasePointZ(i) = CDbl(objBasePoint.Z)

                        HikakuBasePointX(i) = (CDbl(Sokutei.X))
                        HikakuBasePointY(i) = (CDbl(Sokutei.Y))
                        HikakuBasePointZ(i) = (CDbl(Sokutei.Z))

                    End If
                Next
                For Each ShowBasePoint As String() In ListShowBasePoint
                    If objBasePoint.systemlabel = ShowBasePoint(0) And ShowBasePoint(2) = 1 Then

                        Xweight(i) = 1.0# : Yweight(i) = 1.0# : Zweight(i) = 1.0#
                        'Xweight(8) = 1.0# : Yweight(8) = 1.0# : Zweight(8) = 1.0#
                        'Xweight(13) = 1.0# : Yweight(13) = 1.0# : Zweight(13) = 1.0#
                        'Xweight(18) = 1.0# : Yweight(18) = 1.0# : Zweight(18) = 1.0#
                    End If
            ...
            Try
                ...
                'Op.VectorToHomMat3d("rigid", HikakuBasePoint.X, HikakuBasePoint.Y, HikakuBasePoint.Z, MotoBasePoint.X, MotoBasePoint.Y, MotoBasePoint.Z, objHommat3d)
                correspond_3d_3d_weight(HikakuBasePointX, HikakuBasePointY, HikakuBasePointZ, MotoBasePointX, MotoBasePointY, MotoBasePointZ, Xweight, Yweight, Zweight, objHommat3d)
                'Op.AffineTransPoint3D(objHommat3d, HikakuBasePointX, HikakuBasePointY, HikakuBasePointZ, RetBasePointX, RetBasePointY, RetBasePointZ)
                HOperatorSet.AffineTransPoint3d(objHommat3d, HikakuBasePointX, HikakuBasePointY, HikakuBasePointZ, RetBasePointX, RetBasePointY, RetBasePointZ)
            ...
            For Each objBasePoint As measurepoint3dTable In lstBasePoint3d
                For Each Sokutei As measurepoint3dTable In lstmeasurepoint3d
                    If objBasePoint.systemlabel = Sokutei.systemlabel Then
                        'Sokutei.X = Tuple.TupleSelect(HikakuBasePointX, ii)
                        'Sokutei.Y = Tuple.TupleSelect(HikakuBasePointY, ii)
                        'Sokutei.Z = Tuple.TupleSelect(HikakuBasePointZ, ii)
                        Sokutei.X = RetBasePointX(ii)
                        Sokutei.Y = RetBasePointY(ii)
                        Sokutei.Z = RetBasePointZ(ii)
...
#End If
            ...
            Dim MastSystemLab As New List(Of String)
            'MastSystemLab = Split(ReadFrameGrabSetting(9), " ", -1)
            Dim j As Integer = 0
            For Each ShowBasePoint As String() In ListShowBasePoint
                If ShowBasePoint(1) = 1 Then
                    MastSystemLab.Add(ShowBasePoint(0))
                End If
            Next

            'Add Kiryu 20170724 モード3計測時、基準点認識率の表示変更
            If CalcGaibuParam = 3 Then
                For Each objBasePoint As measurepoint3dTable In lstBasePoint3d
                    For Each MastSystemLabCnt As String In MastSystemLab
                        If objBasePoint.systemlabel = MastSystemLabCnt Then
                    ...
                    Next
                Next
                BaseNinsikiHiritu = iOK / MastSystemLab.Count        '(20150806 Tezuka ADD) 基準点認識率
        ...
        Me.Refresh()
    End Sub
    Public Sub frmMeasureResult_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadInfo(Me.Tag)
    End Sub
    ...
    Public Sub SaveCSV_fromDataGridView_InFname(ByVal Fname As String, ByVal DGV As DataGridView, Optional ByVal strHeaderTxt As String = "")
        ...
        With DGV
            ...
            If strHeaderTxt <> "" Then
                strFileBuf = strHeaderTxt & vbNewLine
            End If
            For i = 0 To .RowCount - 1 'Kiryu 2→1修正
                For j = 0 To .ColumnCount - 1
                    If j = .ColumnCount - 1 Then
                        strFileBuf = strFileBuf & .Item(j, i).Value
                    Else
    ...
    Private Sub BtnSokuCsv_Click(sender As Object, e As EventArgs) Handles BtnSokuCsv.Click
        Dim strHeadertxt = "測点名,内部番号,認識,X,Y,Z"
    ...
    Public Sub SetCalcGaibuParam(ByVal val As Integer)
        CalcGaibuParam = val
    End Sub
