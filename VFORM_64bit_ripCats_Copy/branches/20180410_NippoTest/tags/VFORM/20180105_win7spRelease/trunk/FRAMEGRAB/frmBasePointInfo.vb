Public Class frmBasePointInfo
    ...
    Private Sub frmBasePointInfo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        FourTenTGSettingFromFile(TGParam)
    ...
    Private Sub ForuTenTargetToFile()
        Dim strFileString As String = ""
        strFileString = strFileString & TxtdblsetAreaHiritu.Text & vbNewLine
        strFileString = strFileString & TxtdblsetArea012.Text & vbNewLine
        strFileString = strFileString & TxtdblsetArea012_dev.Text & vbNewLine
        strFileString = strFileString & TxtdblsetAreaBigST.Text & vbNewLine
        strFileString = strFileString & txtdilationRadius.Text & vbNewLine
        strFileString = strFileString & TxtminAreaFourTen.Text & vbNewLine
        strFileString = strFileString & TxtmaxAreaFourTen.Text & vbNewLine
        strFileString = strFileString & TxtSTConnectionRimit.Text & vbNewLine
        strFileString = strFileString & (CDbl(Txtcircularity.Text) * 0.01).ToString("#0.00") & vbNewLine

        strFileString = strFileString & "1行目：4点ターゲットの判定値１ 大きい3点の面積と小さい1点の面積の比率（新規）" & vbNewLine
        strFileString = strFileString & "2行目：4点ターゲットの判定値２　大きい3点の面積の最小値（ＰＩＸＥＬ）（新規）" & vbNewLine
        strFileString = strFileString & "3行目：4点ターゲットの判定値３　大きい3点の面積の標準偏差の最大値　　（新規）" & vbNewLine
        strFileString = strFileString & "4行目：基準点（大ＳＴ）の抽出用　最少面積" & vbNewLine
        strFileString = strFileString & "5行目：４点ターゲットの４点の膨張率" & vbNewLine
        strFileString = strFileString & "６行目：４点ターゲット抽出の最小面積（ピクセル）" & vbNewLine
        strFileString = strFileString & "７行目：４点ターゲット抽出の最大面積（ピクセル）" & vbNewLine
        strFileString = strFileString & "８行目：４点の連結上限数" & vbNewLine
        strFileString = strFileString & "９行目：４点の各点における真円度閾値" & vbNewLine

        My.Computer.FileSystem.WriteAllText(My.Application.Info.DirectoryPath & "\Setting\FourTenTarget.txt", strFileString, False)
    ...
    Private Sub CarDetectAndResetSettingToFile()
        ...
        'strFileString = strFileString & MastSystemLab & vbNewLine
    ...
    Private Sub FourTenTGSettingFromFile(ByVal TGParam As TargetDetectParam)

        With TGParam

            TxtdblsetAreaHiritu.Text = .dblsetAreaHiritu.ToString
            TxtdblsetArea012.Text = .dblsetArea012.ToString
            TxtdblsetArea012_dev.Text = .dblsetArea012_dev.ToString
            TxtdblsetAreaBigST.Text = .dblsetAreaBigST.ToString
            txtdilationRadius.Text = .dilationRadius.ToString
            TxtminAreaFourTen.Text = .minAreaFourTen.ToString
            TxtmaxAreaFourTen.Text = .maxAreaFourTen.ToString
            TxtSTConnectionRimit.Text = .STConnectionRimit.ToString
            Txtcircularity.Text = (.circularity * 100).ToString
        End With
    ...
    Private Sub CarDetectAndResetSettingFromFile()
        ...
        Next
        MastSystemLab = ReadFrameGrabSetting(9)
    ...
    Private Sub btnCamGaibuParamReset_Click(sender As System.Object, e As System.EventArgs) Handles btnCamGaibuParamReset.Click

        Dim dbFBMFileNm As String = "dbFBM.mdb"
        Dim CamGaibuPrmFileNm As String = "dbFBM_GaibuPrm.mdb"
        Dim InitialSettingFolderPath As String = My.Application.Info.DirectoryPath & "\Setting\BasePointSearch\InitialSettingFiles\"
        Dim BasePointSearchFolderPath = My.Application.Info.DirectoryPath & "\Setting\BasePointSearch\"

        Try
            Dim ret As DialogResult = MsgBox("カメラ外部評定を初期化します。本当によろしいでしょうか？", MessageBoxButtons.YesNo)
            If ret = DialogResult.Yes Then
                System.IO.File.Copy(InitialSettingFolderPath & CamGaibuPrmFileNm, BasePointSearchFolderPath & dbFBMFileNm, True)
            End If
            ...
            AccessDisConnect()
            FrmMainHiroshima.txtLastBasePointGetTime.Text = lstCamPose(0).CUpdateDate

            MsgBox("カメラ外部評定の初期化が完了しました。")
    ...
    Private Sub btnSysSetFileReset_Click(sender As System.Object, e As System.EventArgs) Handles btnSysSetFileReset.Click

        Dim dbFBMFileNm As String = "dbFBM.mdb"
        Dim CamGaibuPrmFileNm As String = "dbFBM_GaibuPrm.mdb"
        Dim SysMDB As String = "システム設定.mdb"
        Dim SnsrStngFileNm As String = "Sensor_Setting.txt"
        Dim FurTnTrgtFileNm As String = "FourTenTarget.txt"
        Dim FrmGrbStngFileNm As String = "FrameGrabSetting.txt"
        Dim SnsrStng As String = "Sensor_Setting.txt"
        Dim ShwBsePnt As String = "ShowBasePoint.csv"

        Dim InitialSettingFolderPath As String = My.Application.Info.DirectoryPath & "\Setting\BasePointSearch\InitialSettingFiles\"
        Dim SystemFolderPath As String = My.Application.Info.DirectoryPath & "\Setting\"
        Dim BasePointSearchFolderPath = My.Application.Info.DirectoryPath & "\Setting\BasePointSearch\"
        Dim SysSetingMDBFolderPath As String = My.Settings.YCMFolder & "\計測システムフォルダ\"


        Try
            Dim ret As DialogResult = MsgBox("システムの設定を初期化します。本当によろしいでしょうか？", MessageBoxButtons.YesNo)
            If ret = DialogResult.Yes Then
                System.IO.File.Copy(InitialSettingFolderPath & CamGaibuPrmFileNm, BasePointSearchFolderPath & dbFBMFileNm, True)
                System.IO.File.Copy(InitialSettingFolderPath & SnsrStngFileNm, SystemFolderPath & SnsrStngFileNm, True)
                System.IO.File.Copy(InitialSettingFolderPath & FurTnTrgtFileNm, SystemFolderPath & FurTnTrgtFileNm, True)
                System.IO.File.Copy(InitialSettingFolderPath & FrmGrbStngFileNm, SystemFolderPath & FrmGrbStngFileNm, True)
                System.IO.File.Copy(InitialSettingFolderPath & SnsrStng, SystemFolderPath & SnsrStng, True)
                System.IO.File.Copy(InitialSettingFolderPath & ShwBsePnt, SystemFolderPath & ShwBsePnt, True)
                System.IO.File.Copy(InitialSettingFolderPath & SysMDB, SysSetingMDBFolderPath & SysMDB, True)
                System.IO.File.Copy(InitialSettingFolderPath & dbFBMFileNm, My.Application.Info.DirectoryPath & "\" & dbFBMFileNm, True)
