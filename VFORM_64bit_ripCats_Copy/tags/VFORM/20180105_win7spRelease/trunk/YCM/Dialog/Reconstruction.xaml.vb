    ...
    Private Function CreateListReconstData(ByVal imagePairs As List(Of List(Of ImageSet))) As List(Of ReconstData)
        ...
        For Each objReconstData As ReconstData In lstReconstData
            ...
            If objReconstData.flgSuccessGenStereoModel = False Then
                ...
                If objReconstData.GenCameraSetupModel(False, False) = True Then
...
#End If
    ...
    Private Sub RunReconstData(ByRef objReconstdata As ReconstData)
        If objReconstdata.flgSuccessGenStereoModel = True Then
            ...
            End If
        End If
        objReconstdata.flgProcessEnd = True
    End Sub

    Private Sub RunReconstDataByExe(ByRef objReconstdata As ReconstData)
        If objReconstdata.flgSuccessGenStereoModel = True Then
            '  MsgBox("ReconstructScene2")
            Dim p As System.Diagnostics.Process = System.Diagnostics.Process.Start("OnlyReconstructSurfaceStereo.exe",
                                                                                   objReconstdata.pair(0).ImageFullPath & " " & objReconstdata.pair(1).ImageFullPath & " " &
                                                                                   objReconstdata.SettingData.SelectedImageIDs & " " &
                                                                                   MainFrm.objFBM.ProjectPath & " " &
                                                                                   MainFrm.objFBM.pScaleMM)
            p.WaitForExit()
            If System.IO.File.Exists(MainFrm.objFBM.ProjectPath & "\" & objReconstdata.SettingData.SelectedImageIDs & "_object.om3") Then
                HOperatorSet.ReadObjectModel3d(MainFrm.objFBM.ProjectPath & "\" & objReconstdata.SettingData.SelectedImageIDs & "_object.om3", 1,
                                               New HTuple, New HTuple, objReconstdata.SettingData.hv_ResultObject3D, New HTuple)
                'objReconstdata.CalcSmooth(False)
                'objReconstdata.CalcSampling(False)
                'objReconstdata.RunCleanUp(False)
                'objReconstdata.CalcMesh(False)
...
#If True Then
            ...
            Dim ParaOption As New System.Threading.Tasks.ParallelOptions
            ParaOption.MaxDegreeOfParallelism = 8
            ...
            For Each objReconstDataItem As ReconstData In lstReconstData
                ...
                If lstParaReconstData.Count = maxParaCnt Then
                    System.Threading.Tasks.Parallel.For(0, lstParaReconstData.Count, ParaOption, Sub(i)
                                                                                                     'RunReconstData(lstParaReconstData.Item(i))
                                                                                                     RunReconstDataByExe(lstParaReconstData.Item(i))
                                                                                                     progressModal.ProgressUp()
                                                                                                 End Sub)
            ...
            'Next
#Else
            Dim lstTT As New List(Of System.Threading.Thread)
            For Each objReconstDataItem As ReconstData In lstReconstData

                Dim TT As System.Threading.Thread = New Thread(Sub() RunReconstDataByExe(objReconstDataItem))
                TT.Start()
                lstTT.Add(TT)

            Next
            For Each objTT As System.Threading.Thread In lstTT
                objTT.Join()
                progressModal.ProgressUp()
                objTT.Abort()
            Next
...
#End If
    ...
    Public Sub GenStereoModel()
        ...
        HOperatorSet.ReadCameraSetupModel(MainFrm.objFBM.ProjectPath & "\" & SettingData.SelectedImageIDs & ".csm", hv_CameraSetupModelID)
