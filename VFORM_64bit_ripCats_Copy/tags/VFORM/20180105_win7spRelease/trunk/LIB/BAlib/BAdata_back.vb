...
Public Class BAdata_back
    ...
    Public Sub ResetdE_E_F_Gmat()
        ...
        ReDim Gmat(CamGaibuHensu * ImageNum + CamNaibuHensu, CamGaibuHensu * ImageNum + CamNaibuHensu)
    ...
    Public Sub Reconst3D()
        ...
        For i = 1 To PointNum
            ...
            Points(i).X = BTuple.TupleSelect(XYZa, 0)
            Points(i).Y = BTuple.TupleSelect(XYZa, 1)
            Points(i).Z = BTuple.TupleSelect(XYZa, 2)
        Next
    End Sub
...
#End If
    ...
    End Sub
    Private Sub TESTTEMP(ByVal c As Double, ByRef GMatID As Object,
                         ByRef DFMatID As Object, ByRef FallMatID As Object,
                         ByRef EallMatID As Object)
        Dim TGMatID As Object = Nothing
        Dim TDFMatID As Object = Nothing
        Dim TFallMatID As Object = Nothing
        Dim TEallMatID As Object = Nothing

        Parallel.Invoke(
          Sub()
              Calc_G_mat(c, TGMatID)
          End Sub,
          Sub()
              Calc_DF_Mat(TDFMatID)
          End Sub,
          Sub()
              Calc_Fall_mat(TFallMatID)
          End Sub,
          Sub()
              Calc_EaInvAll_mat(c, TEallMatID)
          End Sub)


        GMatID = TGMatID
        DFMatID = TDFMatID
        FallMatID = TFallMatID
        EallMatID = TEallMatID
    ...
    Public Sub Calc_DeltaMat_para2(ByVal c As Double, ByRef DeltaMatID As Object)
        Dim GMatID As Object = Nothing
        Dim DFMatID As Object = Nothing
        Dim FallMatID As Object = Nothing
        Dim EallMatID As Object = Nothing
        ...
        Parallel.For(1, PointNum + 1, Sub(i)
                                          ...
                                          DeltaAEMatID = arrDeltaAEMatID(i)
                                          HOperatorSet.MultMatrix(FaMatID, DeltaEpsFMatID, "AB", MultMatId1)
                                          HOperatorSet.AddMatrixMod(MultMatId1, DeltaAEMatID) ', AddMatID)
                                          HOperatorSet.MultMatrix(EaInvMatID, MultMatId1, "AB", MultMatId2)
                                          HOperatorSet.MultElementMatrixMod(MultMatId2, tmpMatID)
...
#End If
    ...
    Private Sub Calc_G_mat(ByVal c As Double, ByRef GMatID As Object)
        ...
        Dim t As Long = 0
        Dim tmpIndexI As Integer = 1
        Dim tmpIndexJ As Integer = 1
        '     Dim l As Integer

        For i = PointNum * PointHensu + 1 To Shikisuu
            If RemoveRowCol(tmpIndexI) = i Then
                If tmpIndexI < KoteiHensu Then
                    tmpIndexI += 1
                End If
            Else
                tmpIndexJ = 1
                For j = PointNum * PointHensu + 1 To Shikisuu
                    If RemoveRowCol(tmpIndexJ) = j Then
                        If tmpIndexJ < KoteiHensu Then
                            tmpIndexJ += 1
                        End If
                    Else
                        If i = j Then
                            'tmpobj(t) = (1 + c) * ddEmat(i, j)
                            tmpobj(t) = (1 + c) * Gmat(i - PointNum * PointHensu, j - PointNum * PointHensu)
                        Else
                            ' tmpobj(t) = ddEmat(i, j)
                            tmpobj(t) = Gmat(i - PointNum * PointHensu, j - PointNum * PointHensu)
                        End If
                        t = t + 1
                    End If
                Next
...
#If False Then
    ...
    Public Sub BaDataKoushin(ByRef BaMainData As BAdata, ByVal DeltaMatID As Object)
